...
def create_model(hp):
    ...
    if hp.Choice('dropout1', [True, False]):
    ...
    model.add(layers.MaxPooling2D((2, 2)))
    if hp.Choice('dropout2', [True, False]):
        model.add(layers.Dropout(0.5))
    ...
    if hp.Choice('dropoutFC', [True, False]):
        model.add(layers.Dropout(0.5))

    model.add(layers.Dense(8, activation='softmax'))

    optimizer = get_optimizer(hp.Choice('optimizer', ['Adagrad']),
                                   hp.Choice('learning_rate', [1e-2]),
                                    hp.Choice('momentum', [0.2]))
...
def main():
    ...
    path_model = "models/" + backbone + num_of_experiment + '_' + date_start
